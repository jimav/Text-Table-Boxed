...
use Data::Dumper::Interp qw/visnew dvis vis/; ###TEMP for debug

use Carp;
use Scalar::Util qw/reftype/;
use List::Util qw/min max/;
use List::MoreUtils qw/true false any none/;
...
sub parse_rulepicture($) {
  ...
  foreach ($top_rule, $aftertitle_rule, $mid_rule, $bot_rule) {
    ...
    chomp;
    my $rulestr = $_;
btw dvis 'AAA $rulestr pos(rulestr)=',u(pos($rulestr)) if $debug;
...
sub new {
  ...
  $opts{picture} //= $ascii_picture;  # the default
  if (defined $opts{style}) {
      my $options;
      if    ($opts{style} =~ /^ascii$/i)   { 
          $opts{picture} = $ascii_picture; 
      }
      elsif ($opts{style} =~ /^boxrule$/i) { 
          $opts{picture} = $boxrule_picture; 
      }
      else { croak "Invalid 'style' \"$opts{style}\"\n"; }
  }
...
sub add {
    ...
    push @{ $self->{MYKEY}->{row_starts} }, $self->table_height;
...
sub rendered_body_rows {
    my $self = shift;
    return $self->body_rows(@_[0..**BOGUS*], 1);
}
sub rendered_rows {
    my $self = shift;
    my @result = ($self->rendered_title(), $self->rendered_body_rows);
}
...
sub rows {
    ...
    if ($num_rows != 0) {
        if ($row_index == 0) {
            push @result, $self->title_row();
            --$num_rows;
        } else {
            --$row_index;
        }
        push @result, $self->body_rows($row_index, $num_rows);
    }
    return @result if wantarray;
    croak "scalar context valid only for a single row" if @result > 1;
    return $result[0];
}
...
and horizontal rules, taking into account cells containing multiple lines.

Support for ASCII or Unicode box-drawing characters is built in.
Custom ruling is specified with an "ascii art" picture of what you want.
...
within a multi-line row.  Multi-line rows are created when a cell value
...
row represented by an array containing the constituent lines.  This is in
contrast to similar methods without "row" in their name, which return a flat
list of lines (in scalar context, a single possibly-multiline string).

Methods with "B<rendered>" in their name include I<rule lines> in their
processing.  For example B<rendered_table()> returns all lines in the table
including rule lines, whereas B<table()> omits rule lines.

I<Rules> are generated when B<rendered_*()> methods are called, however
...
other substrings are taken as separators including any spaces (e.g. for
...
with single-character datums in each cell.  Characters are replicated to
cover the actual column widths.
...
Like C<table()> but includes rule lines.
    ...
    $line  = $tb->rendered_table($line_index);
...
Returns the entire table including rule lines as a single giant string.
This is also returned via overloading if the object is stringified.
...
Separators and rules may be specified as a "picture" built from several 
...
RENDERING WITH DEGENERATE DATA: 
